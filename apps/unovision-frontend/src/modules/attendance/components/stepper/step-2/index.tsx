...
import {
  type BasicReportInfoSchema,
  basicReportInfoSchema,
...
export default function Step2() {
  ...
  const onSubmit = (formValues: BasicReportInfoSchema) => {
    const userHasChangedPeriodWithoutChangedFile =
      formValues.monthNumber !== fileMetadata?.validatedFilePeriod.monthNumber ||
      formValues.yearNumber !== fileMetadata?.validatedFilePeriod.yearNumber;

    if (userHasChangedPeriodWithoutChangedFile) {
      methods.setError('file', {
        type: 'manual',
        message:
          'El periodo seleccionado no coincide con el periodo del archivo. Por favor, corrige el periodo o el archivo.',
      });
      return;
    }
      ...
      <FormProvider {...methods}>
        <form onSubmit={handleSubmit(onSubmit)} className='w-full flex flex-col gap-8'>
          <div className='w-full overflow-hidden flex flex-col gap-5'>
            <div className='w-full flex items-center flex-col gap-2'>
              ...
              <p className='text-base text-muted-foreground'>
                Completa los datos para el cual deseas generar el reporte de asistencia
              </p>
            </div>
          ...
          <StepperLayout.Footer>
            ...
            <StepperLayout.Button type='submit'>Siguiente</StepperLayout.Button>
          </StepperLayout.Footer>
        </form>
      </FormProvider>
    </StepperLayout.Root>
